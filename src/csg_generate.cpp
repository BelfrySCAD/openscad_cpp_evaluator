...
ColoredBody Evaluator::tagGenerated(manifold::Manifold body, const oscad::ASTNode& node, const Value& colorValue) {
    ...
    if (!measuring_) {
        for (uint32_t originalId : mesh.runOriginalID) {
            ...
            idToCallSite[originalId] = generateUserEntry;
        }
    }
    ColoredBody cb;
